...
/// раздел NES/Dendy, что и metadata::scraper, но другую часть сайта — здесь страницы
/// со списками файлов для скачивания, а не карточки метаданных по отдельным играм.
const ROMS_PATH: &str = "/consoles/dendy/roms";
...
/// Полнотекстовый поиск по разделу NES/Dendy — тот же эндпоинт, что использует
/// metadata::scraper для поиска одной игры по названию, но здесь возвращаем все
/// совпадения (не только первое/точное), чтобы пользователь сам выбрал нужное.
pub async fn search(query: &str) -> Result<Vec<StoreGameSummary>, String> {
    ...
    let html = client
        ...
        .query(&[("id", NES_SECTION_ID), ("q", query)])
        .send()
        .await
...
pub async fn browse_letter(letter: &str, page: u32) -> Result<StoreListingPage, String> {
...
/// поддерживается (см. commands::store::extract_nes_from_zip).
pub async fn list_files(slug: &str) -> Result<Vec<StoreFile>, String> {
    let id = resolve_game_id(slug).await?;
...
pub async fn download_zip(id: &str, fid: &str) -> Result<Vec<u8>, String> {
    let client = build_client()?;
    let bytes = client
        .get(format!("{BASE_URL}{ROMS_PATH}"))
...
pub(crate) async fn resolve_game_id(slug: &str) -> Result<String, String> {
    let client = build_client()?;
    let html = client
        .get(game_page_url(slug))
        .send()
        .await
        .map_err(|e| e.to_string())?
        .error_for_status()
...
pub(crate) fn game_page_url(slug: &str) -> String {
...
fn slug_from_roms_href(href: &str) -> Option<String> {
    let rest = href.strip_prefix(ROMS_PATH)?.strip_prefix('/')?;
...
mod tests {
    ...
    fn extracts_slug_from_game_link() {
        assert_eq!(
            slug_from_roms_href("/consoles/dendy/roms/babel-no-tou"),
    ...
    fn rejects_pagination_links_as_slugs() {
        assert_eq!(slug_from_roms_href("/consoles/dendy/roms/b/2"), None);
        assert_eq!(slug_from_roms_href("/consoles/dendy/roms"), None);
    }
    ...
    async fn lists_real_files_and_downloads_one() {
        let files = list_files("babel-no-tou").await.expect("list_files should succeed");
        let file = files
            .iter()
        ...
        let id = resolve_game_id("babel-no-tou").await.expect("should resolve id");
        let bytes = download_zip(&id, &file.fid).await.expect("download should succeed");
