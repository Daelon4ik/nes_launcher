...
import {
  getSavedGenesisControls,
  saveGenesisControls,
  type GenesisActionName,
  DEFAULT_GENESIS_CONTROLS,
} from "../../utils/genesisControls";
import {
  getSavedGenesisGamepadControls,
...
const getGenesisGamepadIcon = (id: GenesisActionName) => {
  switch (id) {
    case "UP": return <div className={styles.dpadBtn}><ArrowUp size={16} /></div>;
    case "DOWN": return <div className={styles.dpadBtn}><ArrowDown size={16} /></div>;
    case "LEFT": return <div className={styles.dpadBtn}><ArrowLeft size={16} /></div>;
    case "RIGHT": return <div className={styles.dpadBtn}><ArrowRight size={16} /></div>;
    case "A": return <div className={styles.roundBtn} style={{ background: '#3b82f6' }}>A</div>;
    case "B": return <div className={styles.roundBtn} style={{ background: '#f1c40f', color: '#000' }}>B</div>;
    case "C": return <div className={styles.roundBtn} style={{ background: '#e74c3c' }}>C</div>;
    case "START": return <div className={styles.pillBtn}>Start</div>;
    default: return null;
  }
};
...
export function EmulatorTab() {
  const [platform, setPlatform] = useState<"nes" | "genesis">("nes");
  ...
  const [genesisControls, setGenesisControls] = useState(getSavedGenesisControls);
  const [listeningForGenesis, setListeningForGenesis] = useState<{
    player: "player1" | "player2";
    action: GenesisActionName;
  } | null>(null);
  const [genesisGamepadControls, setGenesisGamepadControls] = useState(getSavedGenesisGamepadControls);
  const [listeningForGenesisGamepad, setListeningForGenesisGamepad] = useState<{
    player: "player1" | "player2";
    action: GenesisActionName;
  } | null>(null);
  ...
  useEffect(() => {
    ...
    function handleKeyDown(e: KeyboardEvent) {
      ...
      const { player, action } = listeningForGenesis!;
      setGenesisControls((prev) => {
        const next = { ...prev, [player]: { ...prev[player], [action]: newBinding } };
        saveGenesisControls(next);
        return next;
      });
      setListeningForGenesis(null);
    }
  ...
  // То же самое для Genesis-раскладки геймпада — отдельное состояние/хранилище
  // (utils/genesisGamepad.ts), сам алгоритм ожидания нажатия идентичен NES-версии выше.
  useEffect(() => {
    if (!listeningForGenesisGamepad) return;
    const { player, action } = listeningForGenesisGamepad;
  ...
  const genesisActions: { id: GenesisActionName; label: string }[] = [
    { id: "UP", label: "Вверх" },
    { id: "DOWN", label: "Вниз" },
    { id: "LEFT", label: "Влево" },
    { id: "RIGHT", label: "Вправо" },
  ...
  function handleReset() {
    if (platform === "nes") {
      ...
      if (activeTab === "keyboard") {
        setGenesisControls(DEFAULT_GENESIS_CONTROLS);
        saveGenesisControls(DEFAULT_GENESIS_CONTROLS);
      } else {
        setGenesisGamepadControls(DEFAULT_GENESIS_GAMEPAD_CONTROLS);
        saveGenesisGamepadControls(DEFAULT_GENESIS_GAMEPAD_CONTROLS);
      }
    }
  }
  ...
  return (
    <div className={styles.container}>
      ...
      <div className={styles.tabs}>
        <button
          type="button"
          className={`${styles.tabButton} ${platform === 'nes' ? styles.activeTab : ''}`}
          onClick={() => setPlatform('nes')}
        >
          NES
        </button>
        <button
          type="button"
          className={`${styles.tabButton} ${platform === 'genesis' ? styles.activeTab : ''}`}
          onClick={() => setPlatform('genesis')}
        >
          Genesis
        </button>
      </div>
      ...
      {platform === 'nes' ? (
        ...
        <div className={styles.players}>
          <div className={styles.playerColumn}>
            <h3>Игрок 1</h3>
            {genesisActions.map((a) => (
              <div key={a.id} className={styles.keyRow}>
                <span>{a.label}</span>
                <button
                  type="button"
                  className={`${styles.keyButton} ${
                    listeningForGenesis?.player === "player1" && listeningForGenesis.action === a.id
                      ? styles.listening
                      : ""
                  }`}
                  onClick={() => setListeningForGenesis({ player: "player1", action: a.id })}
                >
                  {listeningForGenesis?.player === "player1" && listeningForGenesis.action === a.id
                    ? "Нажмите..."
                    : genesisControls.player1[a.id].name}
                </button>
              </div>
            ))}
          </div>
            ...
            {genesisActions.map((a) => (
              <div key={a.id} className={styles.keyRow}>
                ...
                <button
                  type="button"
                  className={`${styles.keyButton} ${
                    listeningForGenesis?.player === "player2" && listeningForGenesis.action === a.id
                      ? styles.listening
                      : ""
                  }`}
                  onClick={() => setListeningForGenesis({ player: "player2", action: a.id })}
                >
                  {listeningForGenesis?.player === "player2" && listeningForGenesis.action === a.id
                    ? "Нажмите..."
                    : genesisControls.player2[a.id].name}
                </button>
              </div>
            ))}
          </div>
