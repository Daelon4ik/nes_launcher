...
## Мультиплатформенность (планируется)

Кооп для Sega Genesis (`.gen`) — часть задачи по добавлению этой платформы, см.
[platforms.md](platforms.md#netplay-кооп). Лок-степ протокол сам по себе (буферизация
ввода, `INPUT_DELAY_FRAMES`, `NetplayMessage`) платформо-агностичен; завязка на jsnes —
только в месте, где движок продвигается на кадр и куда пишутся кнопки (см. следующий
раздел). План — `NetplayEngine` работает через тот же интерфейс `EmulatorCore`, что и
`Emulator Screen` в одиночной игре, вместо прямого импорта `jsnes`; формат `Input.buttons`
расширяется, чтобы вместить Genesis-кнопки (сейчас — NES-специфичный 1-байтовый битфлаг).
Ниже описано текущее, NES-only состояние.
