...
Два инстанса лаунчера играют одну NES- или Genesis-игру вместе — один хостит, другой
...
Кооп для Sega Genesis (`.gen`) — см. [platforms.md](platforms.md#netplay-кооп) для деталей
реализации. Лок-степ протокол сам по себе (буферизация ввода, `INPUT_DELAY_FRAMES`,
`NetplayMessage`) платформо-агностичен и не менялся; `NetplayEngine`
(`src/netplay/engine.ts`) диспетчеризует по `platform: "nes" | "genesis"` тем же явным
ветвлением, что и одиночная игра в `Emulator Screen` (`isGenesis`) — не через общий интерфейс
движка (два потребителя, преждевременная абстракция не нужна). Формат `Input.buttons`
остался 1 байтом без изменений — Genesis-раскладка (D-pad + A/B/C/Start = 8 экшенов) как раз
укладывается в те же 8 бит, что и NES-контроллер, просто с другим порядком/семантикой (см.
`src/netplay/genesisLocalInput.ts`). Ниже описан протокол и LAN/Steam-транспорты — они
платформо-агностичны с самого начала, Rust-сторона не менялась.

## Явные ограничения MVP

- Ровно два игрока: хост = контроллер 1, клиент = контроллер 2 (для обеих платформ).
...
1. Читает своё текущее состояние кнопок (клавиатура + любой доступный геймпад) — 1 байт.
   NES (`src/netplay/localInput.ts`): бит = `1 << Controller.BUTTON_*` (0-7; turbo-кнопки
   8/9 не участвуют). Genesis (`src/netplay/genesisLocalInput.ts`): свой фиксированный
   порядок бит `UP/DOWN/LEFT/RIGHT/A/B/C/START` (0-7) — тоже ровно 8 экшенов, тоже 1 байт,
   но не совпадает с внутренним RetroPad-кодированием `GenesisCore.setInput`, это отдельная,
   протокольная нумерация. Обе платформы поддерживают альтернативную WASD-раскладку для
   клавиатуры.
...
3. Смотрит буферизованный ввод **партнёра** для ТЕКУЩЕГО `frameNo`. Если ещё
   ...
   номера кадра.
4. Иначе применяет буферизованный локальный ввод (для `frameNo`) и полученный ввод
   партнёра к соответствующим портам движка и продвигает на кадр — NES:
   `nes.buttonDown/buttonUp`, `nes.frame()`, `screen.writeBuffer()`; Genesis:
   `core.setInput()`, `core.frame()`, рендер RGBA-кадра в свою канву — затем `frameNo++`.
